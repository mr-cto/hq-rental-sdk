...
describe('Emails API', () => {
    ...
    describe('listEmailTemplates', () => {
      it('should call client.get with correct URL', async () => {
        const mockTemplates = [
          ...
          { id: '2', name: 'Confirmation', subject: 'Confirmed', body: 'Your booking is confirmed', type: 'reservation' }
        ];
      ...
      it('should handle templates with optional properties', async () => {
        const mockTemplates = [
          ...
          { id: '2', name: 'Advanced', subject: 'Advanced Template', body: 'Advanced content', type: 'marketing' }
        ];
        mockClient.get.mockResolvedValue(mockTemplates);
  ...
  describe('Reservation Emails', () => {
    describe('sendReservationConfirmation', () => {
      it('should call client.post with correct URL', async () => {
        ...
        expect(mockClient.post).toHaveBeenCalledWith(`/car-rental/reservations/${reservationId}/emails/confirmation`);
        expect(result).toBeUndefined();
      });
    ...
    describe('sendReservationContract', () => {
      it('should call client.post with correct URL', async () => {
        ...
        expect(mockClient.post).toHaveBeenCalledWith(`/car-rental/reservations/${reservationId}/emails/contract`);
        expect(result).toBeUndefined();
      });
    ...
    describe('listReservationEmailTemplates', () => {
      it('should call client.get with correct URL', async () => {
        ...
        const mockTemplates = [
          { id: '1', name: 'Confirmation', subject: 'Booking Confirmed', body: 'Your booking is confirmed' },
          { id: '2', name: 'Reminder', subject: 'Pickup Reminder', body: 'Remember your pickup tomorrow' }
        ];
        mockClient.get.mockResolvedValue(mockTemplates);

        const result = await emailsAPI.listReservationEmailTemplates(reservationId);

        expect(mockClient.get).toHaveBeenCalledWith(`/car-rental/reservations/${reservationId}/emails/templates`);
        expect(result).toEqual(mockTemplates);
      });
      ...
      it('should handle special characters in reservation ID', async () => {
        ...
        const mockTemplates = [
          { id: '1', name: 'Special Template', subject: 'Special Subject', body: 'Special content' }
        ];
        mockClient.get.mockResolvedValue(mockTemplates);
    ...
    describe('triggerReservationEmailTemplate', () => {
      it('should call client.post with correct URL and payload', async () => {
        ...
        expect(mockClient.post).toHaveBeenCalledWith(`/car-rental/reservations/${reservationId}/emails/trigger`, request);
        expect(result).toBeUndefined();
      });
  ...
  describe('default export', () => {
    ...
    it('should have all functions be the same as named exports', () => {
      ...
      expect(emailsAPI.default.sendReservationConfirmation).toBe(emailsAPI.sendReservationConfirmation);
      expect(emailsAPI.default.sendReservationContract).toBe(emailsAPI.sendReservationContract);
      expect(emailsAPI.default.listReservationEmailTemplates).toBe(emailsAPI.listReservationEmailTemplates);
      expect(emailsAPI.default.triggerReservationEmailTemplate).toBe(emailsAPI.triggerReservationEmailTemplate);
  ...
  describe('error handling', () => {
    ...
    it('should propagate errors from triggerEmailTemplate', async () => {
      ...
      await expect(emailsAPI.triggerEmailTemplate({ template_id: 'test' })).rejects.toThrow('Email trigger error');
    });
    ...
    it('should propagate errors from sendReservationConfirmation', async () => {
      ...
      await expect(emailsAPI.sendReservationConfirmation('reservation-id')).rejects.toThrow('Confirmation send error');
    });
    ...
    it('should propagate errors from sendReservationContract', async () => {
      ...
      await expect(emailsAPI.sendReservationContract('reservation-id')).rejects.toThrow('Contract send error');
    });
    ...
    it('should propagate errors from listReservationEmailTemplates', async () => {
      ...
      await expect(emailsAPI.listReservationEmailTemplates('reservation-id')).rejects.toThrow('Reservation templates error');
    });
    ...
    it('should propagate errors from triggerReservationEmailTemplate', async () => {
      ...
      await expect(emailsAPI.triggerReservationEmailTemplate('reservation-id', { template_id: 'test' })).rejects.toThrow('Reservation email trigger error');
    });
