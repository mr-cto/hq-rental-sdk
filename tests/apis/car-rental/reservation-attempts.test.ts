...
describe('Reservation Attempts API', () => {
  ...
  describe('listReservationAttempts', () => {
    it('should call client.get with correct URL without params', async () => {
      const mockResponse = [{ id: 'attempt-1', email: 'test@test.com', status: 'pending', attempted_at: '2024-01-15T10:00:00Z' }] as any[];
    ...
    it('should call client.get with correct URL with params', async () => {
      const mockResponse = [{ id: 'attempt-1', email: 'test@test.com', status: 'completed', attempted_at: '2024-01-15T10:00:00Z' }] as any[];
      const params = { status: 'completed' };
      mockClient.get.mockResolvedValue(mockResponse);

      const result = await reservationAttemptsAPI.listReservationAttempts(params);

      expect(mockClient.get).toHaveBeenCalledWith('/car-rental/reservation-attempts?status=completed');
      expect(result).toBe(mockResponse);
    });
  });
  ...
  describe('findReservationAttemptsByEmail', () => {
    it('should call client.get with correct URL and encoded email', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith('/car-rental/reservation-attempts?email=test%40test.com');
      expect(result).toBe(mockResponse);
    });
  });
